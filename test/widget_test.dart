...
void main() {
  ...
  testWidgets('hint button shows a visual move suggestion', (
    WidgetTester tester,
  ) async {
    final state = GameState();
    state.stock.clear();
    ...
    expect(find.text('Move 6♥ from waste to tableau 1.'), findsOneWidget);
  ...
  testWidgets('restart deal restores the initial deal state', (
    WidgetTester tester,
  ) async {
    final state = GameState();
    final initialStock = state.stock.length;
    ...
    await tester.tap(find.byKey(const Key('stock')));
    await tester.pumpAndSettle();
    expect(state.stock.length, initialStock - 1);
    expect(state.waste, hasLength(1));
    ...
    expect(state.stock.length, initialStock);
    expect(state.waste, isEmpty);
  });
  ...
  testWidgets('statistics dialog shows tracked results after a win', (
    WidgetTester tester,
  ) async {
    final state = _buildAlmostWonState();
